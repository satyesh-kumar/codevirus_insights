...
const Sidebar: React.FC<SidebarProps> = ({ theme }) => {
  return (
    <div className="sticky top-24 flex flex-col gap-1">
      <div className={`flex items-center gap-2 p-2 rounded cursor-pointer group transition-colors ${
        ...
        <div className={`w-7 h-7 flex items-center justify-center rounded transition-colors ${
          theme === 'dark' ? 'bg-slate-800 group-hover:bg-blue-900/30' : 'bg-slate-200 group-hover:bg-blue-100'
        }`}>
      ...
      {topics.map(topic => (
        <div key={topic.id} className={`flex items-center gap-3 p-2 rounded cursor-pointer transition-all border-l-2 border-transparent ${
          theme === 'dark' 
          ...
          <span className="text-sm truncate font-medium">{topic.name}</span>
        </div>
      ))}
